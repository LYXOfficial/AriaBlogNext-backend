...
async def getArchiveInfo(year:int,month:int,startl:int=0,endl:int=None,currentCollection=Depends(getDb)):
    try:
        ...
        end_date=datetime(year+1,1,1).timestamp() if month==12 else datetime(year,month+1,1).timestamp()
        totalCount=await currentCollection.count_documents({"publishTime":{"$gte":start_date,"$lt":end_date}})
