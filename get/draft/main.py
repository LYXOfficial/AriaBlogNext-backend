...
@app.get("/draftBySlug")
async def getDraftBySlug(
    slug: str, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
        draft = await currentCollection.find_one({"slug": slug}, {"_id": 0})
        if draft is None:
            raise HTTPException(
                status_code=404, detail={"message": "fail", "error": "draft not found"}
            )
...
async def getDraftSlugs(currentCollection=Depends(getDb), user=Depends(verify)):
    try:
        ...
        drafts = await draftCursor.to_list(
...
async def searchDraftsByTitleCount(
    title: str, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
