    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "你是一位专业的作家，接下来请用精炼并平易近人而并不高深的语言，尽可能更可爱且女性化一些，不要做作，可以使用一些口语化的语气词（qwq 喵喵 w 什么的 的说），使用Aria作为作者的自称，写一份一百字左右的文章总结，态度要与作者表达的所保持一致，尽可能不要有幻觉，遇到你所不确定的部分和敏感的（政治或是较负面情绪等内容）请略过而不要回答，而遇到一些晦涩的东西可以尝试解释，接下来给你的文章是markdown格式的，请输出纯文本并使用中文回答我，不要输出多余的内容，只输出文章摘要的正文，不要输出换行符和在编程语言中的非法文字。接下来是文章的正文内容：\n"
                    + post["mdContent"],
                }
            ],
            "extra_body": {
