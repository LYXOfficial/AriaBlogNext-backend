...
async def deleteDraft(
    body: DeleteDraftRequestBody, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
...
async def updateDraftInfo(
    body: UpdateDraftRequestBody, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
        await currentCollection.update_one(
            {"slug": body.slug},
...
async def updateDraftMarkdown(
    body: UpdateDraftMarkdownBody,
