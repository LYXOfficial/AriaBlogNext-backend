...
async def pushRenderedHtmlCache(
    body: PushRenderedHtmlCacheRequestBody, currentCollection=Depends(getDb)
):
    try:
        if body.secret != os.environ.get("SECRET"):
            return {"message": "fail", "error": "access failed"}
        await currentCollection.update_one(
            {"slug": body.slug}, {"$set": {"cachedHtml": body.html}}
        )
...
async def deletePost(
    body: DeletePostRequestBody, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
...
async def updatePostInfo(
    body: UpdatePostRequestBody, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
        await currentCollection.update_one(
            {"slug": body.slug},
...
async def updatePostMarkdown(
    body: UpdatePostMarkdownBody, currentCollection=Depends(getDb), user=Depends(verify)
):
    try:
        wordCount = len(
            re.findall(r"\b\w+\b", body.markdown)
            + re.findall(r"[\u4e00-\u9fff]", body.markdown)
        )
        await currentCollection.update_one(
            {"slug": body.slug},
            {
