...
def clean_markdown(text):
    ...
    cleaned_text = re.sub(
        r"^\s{0,3}[-*+]\s", "", cleaned_text, flags=re.M
    )  # 无序列表 - * +
    cleaned_text = re.sub(
        r"^\s{0,3}\d+\.\s", "", cleaned_text, flags=re.M
    )  # 有序列表 1. 2. 3.
    cleaned_text = re.sub(
        r"^#{1,6}\s?", "", cleaned_text, flags=re.M
    )  # 标题 # ## ### 等
    cleaned_text = re.sub(
        r"^\s*\|.*?\|\s*$", "", cleaned_text, flags=re.M
    )  # 表格 | col1 | col2 |
...
for i in os.listdir("importer/posts"):
    with open("importer/posts/" + i, "r", encoding="utf-8") as f:
        ...
        wordCount = len(
            re.findall(r"\b\w+\b", "---".join(textContent.split("---")[2:]))
            + re.findall(r"[\u4e00-\u9fff]", "---".join(textContent.split("---")[2:]))
        )
        ...
        res.append(
            {
                ...
                "tags": (
                    data.get("tags", [])
                    if type(data.get("tags", [])) == list
                    else [data.get("tags")]
                ),
                "category": (
                    data.get("categories", [""])[0]
